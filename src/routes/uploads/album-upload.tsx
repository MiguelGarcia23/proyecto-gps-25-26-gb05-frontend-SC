			...
			<div className="card w-96 shadow-sm p-3 bg-white">
				...
				<div className="card-body">
					<form className="fieldset bg-base-200 border-base-300 roundedbox w-xs border p-4" onSubmit={handleSubmit(onSubmit)}>
						...
						<input type="text" className="input" placeholder="Titulo" {...register('albumName',{required:true})}/>


						<label className="label">Portada</label>
						<input type="file" className="file-input file-input-ghost" accept=".png,.jpg,.jpeg" {...register('albumCover', {required:false})}/>

						<label className="label">Canciones del Álbum</label>
						<select className="select" multiple {...register('albumSong', {required:true})}>
